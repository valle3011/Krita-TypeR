...
def build_sfx_svg(text, font_family, font_size, fill, outline, outline_px,
    ...
    if fill2:
        defs = ('  <defs><linearGradient id="sfxgrad" x1="0" y1="0"'
                ...
                '</linearGradient></defs>\n' % (fill, _xml_escape(str(fill2))))
        fill_ref = "url(#sfxgrad)"
    ...
    # 3) Füllung (oben) – einfarbig oder als Verlauf (fill_ref)
    body += _text_el(
        text_esc, font_esc, font_size, x, y, anchor,
        fill=fill_ref, weight_attr=weight_attr, style_attr=style_attr)

    # Optionale Rotation: alle Lagen zusammen um den Ankerpunkt (x, y) drehen.
    if rotate:
        body = ('  <g transform="rotate(%s %s %s)">\n' % (rotate, x, y)
                + body + '  </g>\n')

    body = defs + body      # defs außerhalb der Rotationsgruppe
